...
namespace algorithm {
  ...
  namespace sg_simple {
    	...
    	private:
      		...
      		struct update {//update structure
				vertex_t target;
				weight_t rank;
      		} __attribute__((__packed__));
      ...
      static bool apply_one_update(unsigned char* vertex_state,unsigned char* 
			...
			if (bsp_phase == niters)// stop returning updates in the final iteration
	  			return false;
			else
	  		return true;
      	}
    ...
    template<typename F>
    unsigned long pagerank<F>::niters;//number of iterations is pre specified.
  }
}
#endif
